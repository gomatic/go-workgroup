...
# │                                                                            │
# │ This Makefile is COPIED VERBATIM into every gomatic Go repo and MUST stay  │
# │ byte-identical to the canonical gomatic/build copy. Editing an in-tree     │
# │ copy forks the toolchain and breaks uniformity — the next `make           │
# │ build-self-update` will silently revert your change.                       │
# │                                                                            │
# │   • Change shared behavior  -> edit it in gomatic/build, then re-sync.     │
...
# from canonical with `make build-self-update` (scripts/self-update.sh). A copy
# is NEVER edited inline — all per-repo divergence (knob overrides like
# COVER_PKGS/BINARIES, hook variables like COVER_GATE/INTEGRATION_PKG, and any
# extra targets) lives in an OPTIONAL Makefile.local, `-include`d at the tail.
#
